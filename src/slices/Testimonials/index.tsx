...
const Testimonials = async ({
  ...
  const testimonials = await Promise.all(
    slice.primary.testimonial_group.map((item) => {
      if (
        isFilled.contentRelationship(item.testimonial_single) &&
        item.testimonial_single.uid
      ) {
        return client.getByUID("testimonial", item.testimonial_single.uid);
      }
    })
  );
